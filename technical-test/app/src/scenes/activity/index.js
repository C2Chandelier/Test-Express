...
const Activities = ({ date, user, project }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const selectedProject = projects.data.find((projectData) => projectData.name === project)
      const selectedActivity = data.find((activity) => activity.projectId === selectedProject?._id)
